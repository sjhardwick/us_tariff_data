...
# ---------- 1) read & stack 2012–2014, compute robust UVs ----------
years <- 2012:2014
...
imports_uv <- imports %>%
  mutate(
    quantity_1    = as.numeric(quantity_1),
    quantity_2    = as.numeric(quantity_2),
    customs_value = as.numeric(customs_value),
...
# pooled medians across 2012–2014
uv_medians_2012_2014 <- imports_uv_clean %>%
  group_by(hts8) %>%
  summarise(
    median_uv_q1 = suppressWarnings(median(uv_q1, na.rm = TRUE)),
    median_uv_q2 = suppressWarnings(median(uv_q2, na.rm = TRUE)),
    .groups = "drop"
  )

# ---------- 2) read tariff UQ codes (use 2013 to match your example) ----------
y_meta <- 2013
...
uv_long_q1 <- uv_medians_2012_2014 %>%
...
uv_long_q2 <- uv_medians_2012_2014 %>%
  left_join(tariffs, by = "hts8") %>%
  transmute(
    hts8,
...
# preview and save
print(head(unit_values))
write_csv(unit_values, "data/temp/unit_values_2012_2014.csv")
